...
Status BlockFetcher::ReadBlockContents_compaction() {
  block_size_ = static_cast<size_t>(handle_.size());
  ...
  if (TryGetFromPrefetchBuffer()) {
...

    {
      ...
      // Actual file read
	  // hkim
	  //printf("[%s::%s::%d] offset { %lu } request size { %zu }\n", __FILE__, __func__, __LINE__, handle_.offset(), block_size_ + kBlockTrailerSize);
      status_ = file_->Read_compaction(handle_.offset(), block_size_ + kBlockTrailerSize,
                            &slice_, used_buf_);
#if 0
