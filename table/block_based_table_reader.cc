...
//    dictionary.
Status ReadBlockFromFile_compaction(
  ...
  Status s = block_fetcher.ReadBlockContents_compaction();
...
Status BlockBasedTable::Open(const ImmutableCFOptions& ioptions,
  ...
  if (!file->use_direct_io()) {
    ...
    prefetch_buffer.reset(new FilePrefetchBuffer());
    s = prefetch_buffer->Prefetch_compaction(file.get(), prefetch_off, prefetch_len);
...
BlockIter* BlockBasedTable::NewDataBlockIterator_compaction(
    Rep* rep, const ReadOptions& ro, const BlockHandle& handle,
    BlockIter* input_iter, bool is_index, GetContext* get_context, Status s) {
  PERF_TIMER_GUARD(new_table_block_iter_nanos);
  ...
  if (s.ok() && block.value == nullptr) {
    ...
    std::unique_ptr<Block> block_value;
    {
      StopWatch sw(rep->ioptions.env, rep->ioptions.statistics,
                   READ_BLOCK_GET_MICROS);
      s = ReadBlockFromFile_compaction(rep->file.get(), nullptr /* prefetch_buffer */,
                            rep->footer, ro, handle, &block_value, rep->ioptions,
                            rep->blocks_maybe_compressed, compression_dict,
                            rep->persistent_cache_options, rep->global_seqno,
                            rep->table_options.read_amp_bytes_per_bit);
...
void BlockBasedTableIterator::SeekToFirst_compaction() {
  SavePrevIndexValue();
  index_iter_->SeekToFirst();
  if (!index_iter_->Valid()) {
    ResetDataIter();
...
void BlockBasedTableIterator::Next_compaction() {
  assert(block_iter_points_to_real_block_);
  data_block_iter_.Next();
  FindKeyForward_compaction();
  //FindKeyForward();
}
...
void BlockBasedTableIterator::InitDataBlock_compaction() {
  ...
  if (!block_iter_points_to_real_block_ ||
    ...
    if (read_options_.readahead_size == 0) {
      if (num_file_reads_ < 2) {
        ...
        table_->get_rep()->file->Prefetch_compaction(data_block_handle.offset(),
                                          readahead_size_);
#if 0
    ...
    BlockBasedTable::NewDataBlockIterator_compaction(rep, read_options_, data_block_handle,
...
void BlockBasedTableIterator::FindKeyForward_compaction() {
