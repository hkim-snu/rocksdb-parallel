...
unsigned long long comp_output_buf_time = 0, comp_output_buf_count = 0;
unsigned long long comp_output_write_time = 0, comp_output_write_count = 0;
// hkim
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
	...
	// hkim
	// calls FlushBlockPolicy::Update (interface class)
	// (table/flush_block_policy.cc)
	// FlushBlockBySizePolicy
	// comes here !!!
	// table/flush_block_policy.cc::Update::33
    auto should_flush = r->flush_block_policy->Update(key, value);
    if (should_flush) {
      assert(!r->data_block.empty());
	  // hkim - calls Flush() -> WriteBlock() -> ... -> write() syscall
      Flush();
...
// hkim
void BlockBasedTableBuilder::Add_compaction(const Slice& key, const Slice& value) {
	// hkim
  ...
  if (IsValueType(value_type)) {
	...
	calclock(local_time, &comp_output_buf_time, &comp_output_buf_count);
	//printf("[%s::%s::%d] thread %d should_flush { %d } \n", __FILE__, __func__, __LINE__, (int)pthread_self(), should_flush);
    if (should_flush) {
      assert(!r->data_block.empty());
	  // hkim - calls Flush() -> WriteBlock() -> ... -> write() syscall
	  clock_gettime(CLOCK_MONOTONIC, &local_time[0]);
      ...
      //Flush();
	  clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	  calclock(local_time, &comp_output_write_time, &comp_output_write_count);
...
// hkim
void BlockBasedTableBuilder::Flush_compaction() {
  Rep* r = rep_;
  assert(!r->closed);
  if (!ok()) return;
  if (r->data_block.empty()) return;
  WriteBlock_compaction(&r->data_block, &r->pending_handle, true /* is_data_block */);
...
// hkim
void BlockBasedTableBuilder::WriteBlock_compaction(BlockBuilder* block,
                                        BlockHandle* handle,
                                        bool is_data_block) {
  WriteBlock_compaction(block->Finish(), handle, is_data_block);
  block->Reset();
}

void BlockBasedTableBuilder::WriteBlock_compaction(const Slice& raw_block_contents,
                                        BlockHandle* handle,
                                        bool is_data_block) {
  ...
  WriteRawBlock_compaction(block_contents, type, handle);
...
// hkim ====================
void BlockBasedTableBuilder::WriteRawBlock_compaction(const Slice& block_contents,
  ...
  assert(r->status.ok());
  // hkim
  // Rep == BlockBasedTableBuilder::Rep (this file, line 252)
  // r->file == WritableFileWriter
  // r->file->Append == WritableFileWriter::Append
  r->status = r->file->Append_compaction(block_contents);
