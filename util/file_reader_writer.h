...
class RandomAccessFileReader {
 ...
 public:
  explicit RandomAccessFileReader(std::unique_ptr<RandomAccessFile>&& raf,
      ...
      : file_(std::move(raf)),
        ...
        for_compaction_(for_compaction) {
			//printf("[%s::%s::%d] filename { %s } for_compaction %d \n", __FILE__, __func__, __LINE__, file_name_.c_str(), for_compaction_);
		}
  ...
  Status Read_compaction(uint64_t offset, size_t n, Slice* result, char* scratch) const;
  Status Read(uint64_t offset, size_t n, Slice* result, char* scratch) const;

  Status Prefetch_compaction(uint64_t offset, size_t n) const {
    return file_->Prefetch_compaction(offset, n);
  }
...
class WritableFileWriter {
 ...
 public:
  // hkim 
  // max_buffer_size_ == writable_file_max_buffer_size == 1MiB
  ...
  // hkim
  Status Append_compaction(const Slice& data);
...
class FilePrefetchBuffer {
 public:
  ...
  Status Prefetch_compaction(RandomAccessFileReader* reader, uint64_t offset, size_t n);
