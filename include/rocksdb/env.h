...
class Env {
 public:
  ...
  // hkim
  virtual void PrefetchQueueData() {
	  return;
  }
  // hkim
...
class RandomAccessFile {
 public:
  ...
  virtual void SetToNextPrefetch(uint64_t offset) { }
  ...
  virtual Status Prefetch_compaction(uint64_t /*offset*/, size_t /*n*/) {
    return Status::OK();
  }
...
class WritableFile {
 public:
  ...
  // hkim
  virtual Status PositionedAppend_compaction(const Slice& /* data */, uint64_t /* offset */) {
    return Status::NotSupported();
  }
