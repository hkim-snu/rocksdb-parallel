...
class PosixEnv : public Env {
 public:
  ...
  virtual Status NewRandomAccessFile(const std::string& fname,
    ...
    if (options.use_mmap_reads && sizeof(void*) >= 8) {
	  ...
	  // hkim
      result->reset(new PosixRandomAccessFile(fname, fd, options, read_queue_));
...
#endif
      result->reset(new PosixWritableFile(fname, fd, options, write_queue_));
      //result->reset(new PosixWritableFile(fname, fd, options));
    } else {
      // disable mmap writes
      EnvOptions no_mmap_writes_options = options;
      no_mmap_writes_options.use_mmap_writes = false;
      result->reset(new PosixWritableFile(fname, fd, no_mmap_writes_options, write_queue_));
 ...
 private:
  ...
  // hkim
  std::shared_ptr<MyReadQueue> read_queue_;
  std::shared_ptr<MyWriteQueue> write_queue_;
  //MyReadQueue read_queue_;
  //MyWriteQueue write_queue_;
};
...
PosixEnv::PosixEnv()
  ...
  // hkim
  // make ReadQueue
  read_queue_.reset(new MyReadQueue());
  write_queue_.reset(new MyWriteQueue());
  // c++14 
  //read_queue_ = std::make_unique<MyReadQueue>();
  //write_queue_ = std::make_unique<MyWriteQueue>();
  //read_queue_ = MyReadQueue();
  //write_queue_ = MyWriteQueue();
}
