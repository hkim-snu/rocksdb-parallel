...
#include <rocksdb/calclock.h>       // hkim
#ifdef DOPROFILE
unsigned long long delay_write_sleep_time=0, delay_write_sleep_count=0;
unsigned long long delay_write_cv_wait_time=0, delay_write_cv_wait_count=0;

unsigned long long handle_buffer_full_time=0, handle_buffer_full_count=0;
unsigned long long sched_flush_time=0, sched_flush_count=0;
unsigned long long delay_write_time=0, delay_write_count=0;
unsigned long long log_sync_wait_time=0, log_sync_wait_count=0;

unsigned long long write_wal_time=0, write_wal_count=0;     // point.1
unsigned long long insert_into_time=0, insert_into_count=0; // point.2
unsigned long long join_batch_group_time=0, join_batch_group_count=0;       // point.3
unsigned long long wait_mt_writer_time=0, wait_mt_writer_count=0;           // point.4
unsigned long long pre_write_time=0, pre_write_count=0;         // point.5
#endif
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  calclock(local_time, &join_batch_group_time, &join_batch_group_count);
  // hkim point.3
#endif
...
#ifdef DOPROFILE
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &wait_mt_writer_time, &wait_mt_writer_count);
        // hkim point.4
#endif
...
#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &pre_write_time, &pre_write_count);
    // hkim point.5
#endif
...
#ifdef DOPROFILE
      clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
      calclock(local_time, &write_wal_time, &write_wal_count);
      // hkim point.1
#endif
...
#ifdef DOPROFILE
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &insert_into_time, &insert_into_count);
        // hkim point.2
#endif
...
#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &handle_buffer_full_time, &handle_buffer_full_count);
    // hkim point.1
#endif
  }
...
#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &sched_flush_time, &sched_flush_count);
      // hkim point.2
#endif
  }

  // hkim - write stall because of flush/compaction
...
#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &delay_write_time, &delay_write_count);
    // hkim point.3
#endif
  }
...
#ifdef DOPROFILE
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &log_sync_wait_time, &log_sync_wait_count);
        // hkim point.4
#endif
    }
    for (auto& log : logs_) {
...
Status DBImpl::DelayWrite(uint64_t num_bytes,
  ...
  bool delayed = false;
  {
    ...
    if (delay > 0) {
	  ...
	  //printf("[%s::%s::%d] stall_end { %lu } delay { %lu } \n", __FILE__, __func__, __LINE__, stall_end, delay);
	  // hkim - Due to Compaction
#if 1
...
#if 0
		struct timespec local_time[2];
		unsigned long long prefetch_time=0, prefetch_count=0;
        clock_gettime(CLOCK_MONOTONIC, &local_time[0]);
#endif
		// prefetch sst file contents
		env_->PrefetchQueueData();
#if 0
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &prefetch_time, &prefetch_count);

		//printf("[%s::%s::%d] time { %llu } \n", __FILE__, __func__, __LINE__, prefetch_time);
#endif
      }
#endif
#if 0
...
#ifdef DOPROFILE
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &delay_write_sleep_time, &delay_write_sleep_count);
#endif
      }
#endif
...
#ifdef DOPROFILE
        clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
        calclock(local_time, &delay_write_cv_wait_time, &delay_write_cv_wait_count);
#endif
    }
  }
