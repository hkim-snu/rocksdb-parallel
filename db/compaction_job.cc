...
#ifdef DOPROFILE
unsigned long long proc_kv_comp_time=0, proc_kv_comp_count=0;

unsigned long long make_input_iter_time=0, make_input_iter_count=0;
unsigned long long seek_first_time=0, seek_first_count=0;
unsigned long long add_to_builder_time=0, add_to_builder_count=0;
unsigned long long next_time=0, next_count=0;
unsigned long long finish_comp_output_time=0, finish_comp_output_count=0;
#endif
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
	...
	ProcessKeyValueCompaction_internal(sub_compact);

#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &proc_kv_comp_time, &proc_kv_comp_count);
#endif
}

inline Slice ExtractUserKey_comp_mod(const Slice& internal_key) {
  assert(internal_key.size() >= 8);
  return Slice(internal_key.data(), internal_key.size() - 8);
}

inline int convert_key(std::string _key) {
	return std::stoi(_key, 0, 16);
}

void CompactionJob::ProcessKeyValueCompaction_internal(SubcompactionState* sub_compact) {
  assert(sub_compact != nullptr);
  ColumnFamilyData* cfd = sub_compact->compaction->column_family_data();
  // hkim TODO check RangeDelAggregator
  ...
  // hkim
  // key function !!!!!
  // make iterator from all compaction input files
  // TODO check time
  ...
  printf("OP_MAKE_INPUT_ITER_START %llu thread %d \n", start_ts, (int)pthread_self());
#endif
...
#ifdef DOPROFILE
  clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
  calclock(local_time, &make_input_iter_time, &make_input_iter_count);
#endif
  ...
  printf("OP_MAKE_INPUT_ITER_END %llu %llu thread %d \n", end_ts, (end_ts - start_ts), (int)pthread_self());
#endif
  ...
  // ThreadStatus defined @ include/rocksdb/thread_status.h
  // Operation Stage (for compaction)
  ...
  if (start != nullptr) {
    ...
    input->Seek(start_iter.GetInternalKey());
	  ...
	  // calls table/merging_iterator.cc::SeekToFirst::93
	  // read first block from input file =====================================
	  // calls MergeIterator::SeekToFirst() (table/merging_iterator.cc::SeekToFirst)
	  // calls BlockBasedTableIterator::SeekToFirst (table/block_based_table_reader.cc::SeekToFirst)
	  // calls RandomAccessFileReader::Read()
	  // calls ReadaheadRandomAccessFile::Read()
	  // calls ReadaheadRandomAccessFile::ReadIntoBuffer()
#if 0
	  clock_gettime(CLOCK_MONOTONIC, &tv[0]);
	  start_ts = tv[0].tv_sec*BILLION + tv[0].tv_nsec;

	  printf("OP_INPUT_SEEK_TO_FIRST_START %llu\n", start_ts);
#endif
...
// hkim
	  input->SeekToFirst_compaction();
	  //input->SeekToFirst();

#ifdef DOPROFILE
	  clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	  calclock(local_time, &seek_first_time, &seek_first_count);
#endif

#if 0
	  clock_gettime(CLOCK_MONOTONIC, &tv[1]);
	  end_ts = tv[1].tv_sec*BILLION + tv[1].tv_nsec;

	  printf("OP_INPUT_SEEK_TO_FIRST_END %llu %llu\n", end_ts, (end_ts - start_ts));
#endif
  // read first block from input file =====================================
  }
  ...
  // hkim
  //printf("[%s::%s::%d] \n", __FILE__, __func__, __LINE__);
  // calls compaction_iterator.cc::SeekToFirst::103
  c_iter->SeekToFirst();

  // hkim
  // meaningless code... since it does not check the return value of "ShouldStopBefore()"
  if (c_iter->Valid() &&
    ...
    // first key. So we do that here.
	  ...
	  // hkim
	  // ShouldStopBefore() returns whether stop building the current output
	  // before processing "internal_key"
	  // why this code doesn't check the return value of the function????
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    if (sub_compact->builder == nullptr) {
		...
		// comes here !!!!
		// 
		//sets sub_compact->builder.reset(NewTableBuilder())
		//printf("[%s::%s::%d] \n", __FILE__, __func__, __LINE__);
	...
	// hkim
	// insert (key,value) to compaction builder =========================================
    // TODO check time
    assert(sub_compact->builder != nullptr);
    assert(sub_compact->current_output() != nullptr);
	// hkim - add key/value to merge buffer
	// 1) add (key, value) to builder
	// 2) update output file size
	// 3) update boundary 
	// print-out sorted user key
	//printf("[%s::%s::%d] Key { %s }\n", __FILE__, __func__, __LINE__, key.ToString(true).c_str());
	//Slice _uKey = ExtractUserKey(key);
	//printf("[%s::%s::%d] userKey { %s }\n", __FILE__, __func__, __LINE__, _uKey.ToString(true).c_str());

#if 0
	clock_gettime(CLOCK_MONOTONIC, &tv[0]);
	start_ts = tv[0].tv_sec*BILLION + tv[0].tv_nsec;

	printf("OP_ADD_TO_BUILDER_START %llu\n", start_ts);
#endif
...
// hkim
    sub_compact->builder->Add_compaction(key, value);
    //sub_compact->builder->Add(key, value);
#ifdef DOPROFILE
	clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	calclock(local_time, &add_to_builder_time, &add_to_builder_count);
#endif
	...
	printf("OP_ADD_TO_BUILDER_END %llu %llu\n", end_ts, (end_ts - start_ts));
#endif

	// hkim
	// compression ======================================================================
	// purpose of sampling...? 
	// following values are used for ... compression
	// don't care
	// sample_begin_offset_iter, sample_end_offset
	// dict_sample_data
	...
	// hkim
	// final stage of processing key value
	// 1) check output file size 
	// -> if file size reaches at max value, do 3)
	// 2) find next key by calling c_iter->Next()
	// 3) (output_file_ended) -> 
	// 3.1) call FinishCompactionOutputFile()
	// 3.2) call RecordDroppedKeys() -> purpose???
	// 3.3) update compression dictionary to dict_sample_data
	...
	// ---> BlockBasedTableIterator::FindKeyForward
	// ----> BlockBasedTableIterator::InitDataBlock
...
// hkim
    c_iter->Next_compaction();
    //c_iter->Next();

#ifdef DOPROFILE
	clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	calclock(local_time, &next_time, &next_count);
#endif
	...
	// hkim
	// finishing compaction output file ================================================
    if (output_file_ended) {
	  ...
	  printf("OP_SYNC_FILE_START %llu\n", start_ts);
#endif

	  // write meta blocks to the end of the output file
	  // builder.reset()
	  ...
	  printf("OP_SYNC_FILE_END %llu %llu\n", end_ts, (end_ts - start_ts));
#endif

      RecordDroppedKeys(range_del_out_stats,
                        &sub_compact->compaction_job_stats);
	  // hkim
	  // (don't care) update compression dictionary =====================================
	...
	// finishing compaction output file ================================================
	// check time to file close every key value processing ================================
  }
  // start iteration ==========================================================

#if 0
  // hkim
  printf("output files [%zu] (level : number): ", sub_compact->outputs.size());
  for (auto _out : sub_compact->outputs) {
	  //printf("(%d %lu) ", sub_compact->compaction->output_level(), _out.meta.fd.GetNumber());
	  printf("(%d %lu) [%d %d)\n", sub_compact->compaction->output_level(), _out.meta.fd.GetNumber(), convert_key(ExtractUserKey_comp_mod(_out.meta.smallest.user_key()).ToString(true)), convert_key(ExtractUserKey_comp_mod(_out.meta.largest.user_key()).ToString(true)));
  }
  // hkim
#endif
...
Status CompactionJob::FinishCompactionOutputFile(
    ...
    Status result = FinishCompactionOutputFile_internal(input_status, sub_compact, range_del_agg, range_del_out_stats, next_table_min_key);

#ifdef DOPROFILE
    clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
    calclock(local_time, &finish_comp_output_time, &finish_comp_output_count);
#endif
...
Status CompactionJob::FinishCompactionOutputFile_internal(
    const Status& input_status, SubcompactionState* sub_compact,
    RangeDelAggregator* range_del_agg,
    CompactionIterationStats* range_del_out_stats,
    const Slice* next_table_min_key /* = nullptr */) {
  ...
  if (s.ok()) {
	// update boundary of the compaction output file ===================================
...
Status CompactionJob::OpenCompactionOutputFile(
  ...
  // hkim
  size_t block_size = 0;
  size_t last_allocated_block = 0;
  writable_file->GetPreallocationStatus(&block_size, &last_allocated_block);
  // hkim block_size = 73819750 (70MiB)
  //printf("[%s::%s::%d] PreallocationBlockSize %zu\n", __FILE__, __func__, __LINE__, block_size);
  // hkim
  ...
  // builder == TableBuilder object ... interface class 
  // connect with BlockBasedTableBuilder???
  // (table/block_based_table_factory.cc)
  // class BlockBasedTableFactory::NewTableBuilder
