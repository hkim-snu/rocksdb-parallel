...
unsigned long long make_merge_iter_time=0, make_merge_iter_count=0;
// hkim
...
class LevelIterator final : public InternalIterator {
 public:
  ...
  virtual void SeekToFirst_compaction() override;
...
// hkim
void LevelIterator::SeekToFirst_compaction() {
  InitFileIterator(0);
  if (file_iter_.iter() != nullptr) {
    file_iter_.SeekToFirst_compaction();
  }
  SkipEmptyFileForward();
}
// hkim
...
inline Slice ExtractUserKey_mod(const Slice& internal_key) {
  assert(internal_key.size() >= 16);
  return Slice(internal_key.data(), internal_key.size() - 16);
}

inline int convert_user_key(std::string _key) {
...
InternalIterator* VersionSet::MakeInputIterator(
  ...
  size_t num = 0;
  // hkim
  //printf("input files (level:number) : ");
  for (size_t which = 0; which < c->num_input_levels(); which++) {
    if (c->input_levels(which)->num_files != 0) {
      if (c->level(which) == 0) {
        const LevelFilesBrief* flevel = c->input_levels(which);
		// hkim
		//printf("[LEVEL(%zu) TOTAL(%zu)] : ", which, flevel->num_files);
        for (size_t i = 0; i < flevel->num_files; i++) {
			// hkim
			//printf("(%zu:%lu) [%d %d) \n ", which, flevel->files[i].fd.GetNumber(), convert_user_key(ExtractUserKey_mod(flevel->files[i].smallest_key).ToString(true)), convert_user_key(ExtractUserKey_mod(flevel->files[i].largest_key).ToString(true)) );

			// per-file iterator for input, level0 (NewIterator)
			// (db/table_cache.cc)
			// TableCache::NewIterator()
			// calls pread near the end the file about 512KiB size
...
#if 0
		  const LevelFilesBrief* flevel = c->input_levels(which);
		  printf("[LEVEL(%d) TOTAL(%zu)] : ", c->level(which), flevel->num_files);
		  for (size_t i = 0; i < flevel->num_files; i++) {
			  printf("(%d:%lu) [%d %d) \n ", c->level(which), flevel->files[i].fd.GetNumber(), convert_user_key(ExtractUserKey_mod(flevel->files[i].smallest_key).ToString(true)), convert_user_key(ExtractUserKey_mod(flevel->files[i].largest_key).ToString(true)) );
			  //printf("(%zu:%lu) [%s %s) \n ", which, flevel->files[i].fd.GetNumber(), ExtractUserKey_mod(flevel->files[i].smallest_key).ToString(true).c_str(), ExtractUserKey_mod(flevel->files[i].largest_key).ToString(true).c_str() );
		  }
		  printf("\n");
#endif
		  // per-file iterator for input, Non-level0 (LevelIterator)
  ...
  assert(num <= space);
  // merge all per-file iterator, NewMergingIterator
  // hkim
#if 0
  //unsigned long long prev_time = make_merge_iter_time;
  struct timeval tv;
  gettimeofday(&tv, NULL);
  unsigned long long current_ts = tv.tv_sec*1000LL + tv.tv_usec/1000;
#endif
  ...
  calclock(local_time, &make_merge_iter_time, &make_merge_iter_count);

#if 0
  printf("NewMergingIterator timestamp %llu duration %llu\n", current_ts, (make_merge_iter_time - prev_time));
#endif
  delete[] list;
  return result;
}
