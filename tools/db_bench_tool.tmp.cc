...
#ifdef DOPROFILE
// hkim
unsigned long long batch_put_time=0, batch_put_count=0;
unsigned long long db_write_time=0, db_write_count=0;

unsigned long long do_write_time=0, do_write_count=0;

unsigned long long bench_run_time=0, bench_run_count=0;
// hkim
#endif
...
void VerifyDBFromDB(std::string& truth_db_name) {
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
	...
	calclock(local_time, &do_write_time, &do_write_count);
#endif
  }
...
#ifdef DOPROFILE
	clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	calclock(local_time, &batch_put_time, &batch_put_count);
#endif
...
#ifdef DOPROFILE
	clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	calclock(local_time, &db_write_time, &db_write_count);
#endif
...
#ifdef DOPROFILE
  clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
  calclock(local_time, &bench_run_time, &bench_run_count);
#endif
